...
# Try to import the full app first (with AI models), fall back to lite_app
# Vercel Python runtime expects 'app' to be available at module level
app = None

# First, try to import the full app with AI models
try:
    print("[api/index.py] Attempting to import full app with AI models...")
    from app import app as full_app
    app = full_app
    print("[api/index.py] Successfully imported full app with AI models!")
except ImportError as e:
    print(f"[api/index.py] Full app import failed (expected in serverless): {e}")
    print("[api/index.py] Falling back to lite_app...")

    # Fall back to lite_app
    try:
        # First, try importing as a module from the chatbot directory
        from lite_app import app as lite_app
        app = lite_app
        print("[api/index.py] Successfully imported lite_app (fallback)")
    except ImportError as e:
        ...
        try:
            from chatbot.lite_app import app as lite_app
            app = lite_app
            print("[api/index.py] Successfully imported chatbot.lite_app (fallback)")
            ...
            try:
                ...
                if spec and spec.loader:
                    ...
                    print("[api/index.py] Successfully imported lite_app via file path (fallback)")
                else:
                ...
                traceback.print_exc()
                raise ImportError(f"Failed to import any app: {e}. Alternative attempts also failed: {e2}, {e3}") from e
