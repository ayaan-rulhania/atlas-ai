...
class PathManager:
    ...
    def __init__(self):
        self.base_dir = Path(__file__).parent.parent.resolve()  # apps/chatbot/app_utils/ -> apps/chatbot/
        self.atlas_root = self.base_dir.parent.parent  # apps/chatbot/ -> apps/ -> root
        self.thor_1_0_dir = self.atlas_root / "models" / "thor-1.0"
        self.thor_1_1_dir = self.atlas_root / "models" / "thor-1.1"
        self.thor_1_2_dir = self.atlas_root / "models" / "thor" / "thor-1.2"
        self.chatbot_dir = self.base_dir
        
        # Track which thor version is currently active
        self._active_thor = 'thor-1.2'
    
    def get_thor_dir(self, version: str = 'thor-1.2') -> Path:
        """Get the path to a specific thor version directory."""
        if version == 'thor-1.0':
            return self.thor_1_0_dir
        elif version == 'thor-1.1':
            return self.thor_1_1_dir
        elif version == 'thor-1.2':
            return self.thor_1_2_dir
        else:
    ...
    def set_active_thor(self, version: str):
        ...
        if version not in ['thor-1.0', 'thor-1.1', 'thor-1.2']:
            raise ValueError(f"Unknown thor version: {version}")
