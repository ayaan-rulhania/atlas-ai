...
```javascript
const { api } = require('atlas-ai-sdk');

// Generate and register a key for Thor 1.2 (recommended default)
const keySuffix = await api('thor-1.2');
// This will print: thor-1.2-AbCdEfGhIjKlMnOpQrStUvWxYz

// Use the key in your code
const API_KEY = `thor-1.2-${keySuffix}`;
```

### Make API Calls

```javascript
const { call } = require('atlas-ai-sdk');
...
```javascript
const { AtlasClient } = require('atlas-ai-sdk');
...
```javascript
import { api, call, AtlasClient } from 'atlas-ai-sdk';
...
<body>
  <script type="module">
    import { api, call } from './node_modules/atlas-ai-sdk/src/index.js';
...
- `model`: `"thor-1.0"`, `"thor-1.1"`, `"thor-1.2"`, or `"antelope-1.1"`
