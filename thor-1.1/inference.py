...
class AllRounderInference:
    ...
    def __init__(self, model_path: str, tokenizer_path: str, config_path: str = 'config/config/config.yaml'):
        ...
        print(f"Enhanced features: Improved context handling, better text generation")
    ...
    def predict(
        ...
        if task == 'text_classification' or task == 'sentiment_analysis':
            ...
            try:
                ...
                # Thor 1.1 Enhanced: Use temperature sampling for more diverse outputs
                # Apply temperature to logits for better generation diversity
                temperature = 0.7  # Lower = more focused, higher = more creative
                next_token_logits = logits[0, -1, :] / temperature  # [vocab_size]
                
                # Apply top-k filtering (keep top 50 tokens) for better quality
                top_k = 50
                if top_k > 0:
                ...
                if not generated_text or generated_text.strip() == text.strip():
                    ...
                    result['version'] = THOR_VERSION  # Mark as Thor 1.1 output
